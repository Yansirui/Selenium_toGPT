...
def get_download_version(c_version):
    r = requests.get("https://chromedriver.storage.googleapis.com/?delimiter=/&prefix=",proxies=proxy_data)
...
def json_version_extractor(vrsn):
    r = requests.get("https://googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json",proxies=proxy_data)
    data = r.json()
...
def chromeDriverDownloader():
    ...
    if curPlt == 'win32':
        ...
        if not os.path.exists('./chromedriver'):
            try:
                #todl = os.popen(r'reg query "C:\Users\17459\AppData\Local\Chromium\Application" /v version')
                ...
                r = requests.get(url, allow_redirects=True,proxies=proxy_data)
